    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      if (token) {
        try {
          ...
          if (exp - now < 60) {
            const refreshData = await refreshInstance.post('/', {}, { headers: { Authorization: `Bearer ${token}` } });
            localStorage.setItem('accessToken', refreshData.data.accessToken);
            const newRequest = { ...originalRequest };
            newRequest.headers = { ...newRequest.headers, Authorization: `Bearer ${refreshData.data.accessToken}` };
